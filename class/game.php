	...
	function values($datas)
	{
		...
		foreach ($datas['games'] as $key => $value) 
		{
    		...
    		{ 
				...
				if  ( !isset($value['stats']['championsKilled']) )  {	$this->kill = 0 ; } 	  else { $this->kill   = $value['stats']['championsKilled'] ; }
				if  ( !isset($value['stats']['numDeaths'])       )  {	$this->death = 0 ; } 	  else { $this->death   = $value['stats']['numDeaths'] ; }
				if  ( !isset($value['stats']['assists'])         )  {	$this->assists = 0 ; }	  else { $this->assists   = $value['stats']['assists'] ; }
				if  ( !isset($value['stats']['timePlayed'])      )  {	$this->time = 0 ; } 	  else { $this->time   = $value['stats']['timePlayed'] ; }
				if  ( !isset($value['stats']['minionsKilled'])   )  {	$this->cs = 0 ; } 		  else { $this->cs   = $value['stats']['minionsKilled'] ; }
				if  ( !isset($value['stats']['sightWardsBought']))  {	$this->pinkBought = 0 ; } else { $this->pinkBought   = $value['stats']['sightWardsBought'] ; }
				if  ( !isset($value['stats']['wardPlaced']) 	 )  {	$this->wardPlaced = 0 ; } else { $this->wardPlaced   = $value['stats']['wardPlaced'] ; }
